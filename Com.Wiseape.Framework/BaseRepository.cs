...
namespace Com.Wiseape.Framework
{
    public abstract class BaseRepository<T> : IRepository where T : class
    {
        ...
        public virtual List<Object> FindAll(SelectParam selectParam, int? page, int? size)
        {
            ...
            {
                if(selectParam != null && selectParam.Where != null)
                    items = context.Set<T>().Where(selectParam.Where).OrderBy( selectParam.OrderByString ).ToList<object>();
                else if (selectParam != null)
                    items = context.Set<T>().OrderBy( selectParam.OrderByString).ToList<object>();
                else
                    items = context.Set<T>().ToList<object>();
            }
            else
            {
                if (selectParam != null && selectParam.Where != null)
                    items = context.Set<T>().Where(selectParam.Where).OrderBy(selectParam.OrderByString).Skip((int)(page - 1) * (int)size).Take((int)size).ToList<object>();
                else if (selectParam != null)
                    items = context.Set<T>().OrderBy(selectParam.OrderByString).Skip((int)page * (int)size).Take((int)size).ToList<object>();
                else
                    items = context.Set<T>().ToList<object>();
            }
